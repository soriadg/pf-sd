...
Environment variables:
  ...
  DB_USER=app_user
  DB_PASS=123456789
  START_PROF_WEBSERVER=0|1
  PROF_WEBSERVER_PORT=8090
EOF
}
...
start_prof_webserver() {
  local name="prof-webserver"
  local dir="$BASE_DIR/prof-webserver"
  local port="$PROF_WEBSERVER_PORT"
  local pid_file="$PID_DIR/$name.pid"
  local log_file="$LOG_DIR/$name.log"
  local jar
  ...
  if is_port_listening "$port"; then
    echo "Port $port already in use. Skipping $name start."
    return 0
  fi
  ...
  (cd "$dir" && nohup java -jar "$jar" "$port" > "$log_file" 2>&1 & echo $! > "$pid_file")
}
...
start_all() {
  ...
  if [ "$START_PROF_WEBSERVER" = "1" ]; then
    echo "ProfWebServer: http://localhost:${PROF_WEBSERVER_PORT}/status"
  fi
}
